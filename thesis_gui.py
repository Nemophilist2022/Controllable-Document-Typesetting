...
class FormatterGUI:
    ...
    def _build_bottom(self, root):
        ...
        # B3: experimental AI review button — drives the thesis-agent
        # harness in eval_only mode and pops up a completion report.
        # Lives next to the main action button so users discover it
        # naturally; never replaces or alters the legacy flow.
        self._btn_ai_review = self._create_button(
            action_frame,
            text="AI 评测（实验性）",
        ...
        self._btn_ai_review.grid(row=5, column=0, sticky="ew", pady=(8, 0))
    ...
    # ------------------------------------------------------------------
    # B3: AI review (experimental)
    #
    # Runs the thesis-agent harness in mode=eval_only on the chosen
    # input file and pops up a four-bucket completion report. We use
    # eval_only on purpose:
    # - no docx is written (R8.2)
    # - no Word COM postprocess is invoked
    # - the user's existing legacy flow stays untouched
    # If mode=full is wanted later it's a one-line change here.
    # ------------------------------------------------------------------
    ...
    def _start_ai_review(self):
        ...
        if not inp:
            self._messagebox.showwarning(
                "AI 评测", "请先在“输入文件”里选择一个 docx / doc / txt / md / tex"
            )
            return
        if not os.path.isfile(inp):
            self._messagebox.showerror("AI 评测", f"输入文件不存在：{inp}")
            return

        self._btn_ai_review.config(state="disabled")
        self._set_status("AI 评测中…")
        self._append_log(f"\n=== AI 评测开始（mode=eval_only）输入: {inp} ===")

        import tempfile

        # Late import keeps GUI startup fast and avoids forcing the
        # thesis_agent import chain on users who never click this button.
        try:
            from thesis_agent.orchestrator.harness import RunOptions, run as agent_run
        except Exception as exc:
            self._append_log(f"AI 评测组件未就绪: {exc}")
            self._btn_ai_review.config(state="normal")
            self._set_status("AI 评测失败")
            return

        out_dir = tempfile.mkdtemp(prefix="thesis_agent_review_")

        def safe_after(callback):
            """Schedule *callback* on the Tk main thread.

            If the root window has already been torn down (e.g. unittest
            tearDown ran while the worker was still in flight) tkinter
            raises ``RuntimeError: main thread is not in main loop``.
            We swallow it: any UI update at that point is moot anyway.
            """
            try:
            ...
            try:
                result = agent_run(
                    ...
                    options=RunOptions(
                        ...
                        llm_disabled=True,  # GUI v0.1: no LLM call
                        log_fn=self._append_log,
                    ),
                )
                self._append_log(
                    f"AI 评测完成: {result.summary} (报告: {result.report_md_path})"
                )
                safe_after(lambda r=result: self._show_ai_report(r))
                safe_after(lambda: self._set_status("AI 评测完成"))
            except Exception as exc:
                self._append_log(f"AI 评测异常: {exc}")
                safe_after(lambda: self._set_status("AI 评测失败"))
            finally:
                safe_after(lambda: self._btn_ai_review.config(state="normal"))

        threading.Thread(target=worker, daemon=True).start()

    def _show_ai_report(self, run_result):
        """Render report.json's four-bucket structure in a Toplevel.

        We deliberately keep this simple: a notebook with a tab per
        status bucket plus a "summary" tab. Each item shows rule_id,
        evidence and the operator-facing advice line.
        """
        import json
        import tkinter as tk

        try:
            with open(run_result.report_json_path, "r", encoding="utf-8") as fh:
                report = json.load(fh)
        except Exception as exc:
            self._messagebox.showerror("AI 评测", f"无法读取报告: {exc}")
            return

        win = tk.Toplevel(self._root)
        win.title("AI 评测 — 完成度报告")
        win.geometry("780x560")

        # Summary header
        summary = report.get("summary", {})
        header = self._ttk.Frame(win, padding=(16, 12))
        header.pack(fill="x")
        self._ttk.Label(
            header,
            text=(
                f"模板：{report.get('profile', '')}    "
                f"模式：{report.get('mode', '')}    "
                f"迭代：{report.get('iterations', 0)}"
            ),
            style="Meta.TLabel",
        ).pack(anchor="w")
        self._ttk.Label(
            header,
            text=(
                f"汇总：✅ 已完成 {summary.get('done', 0)}    "
                f"⚠ 部分完成 {summary.get('partial', 0)}    "
                f"❌ 未完成 {summary.get('failed', 0)}    "
                f"⏭ 已跳过 {summary.get('skipped', 0)}"
            ),
            style="Value.TLabel",
        ).pack(anchor="w", pady=(4, 0))

        # Notebook: one tab per bucket
        nb = self._ttk.Notebook(win)
        ...
        for status_key, title in labels:
            tab = self._ttk.Frame(nb)
            nb.add(tab, text=f"{title} ({sum(1 for it in items if it.get('status') == status_key)})")
            self._render_bucket(tab, [it for it in items if it.get("status") == status_key])

        # Footer with the report.md / json paths so users can open
        # them in another tool if they want.
        footer = self._ttk.Frame(win, padding=(16, 0))
        footer.pack(fill="x", pady=(0, 12))
        self._ttk.Label(
            footer,
            text=f"报告文件: {run_result.report_md_path}",
            style="Hint.TLabel",
        ).pack(anchor="w")
        self._ttk.Label(
            footer,
            text=f"机读 JSON: {run_result.report_json_path}",
            style="Hint.TLabel",
        ).pack(anchor="w")

    def _render_bucket(self, parent, items):
        """Pour items into a scrollable text area."""
        ...
        for it in items:
            ...
            txt.insert("end", f"• {rule_id}  [{severity}]\n")
            if evidence:
                txt.insert("end", f"    {evidence}\n")
            if advice:
                txt.insert("end", f"    建议：{advice}\n")
            txt.insert("end", "\n")
